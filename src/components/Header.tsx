...
export const Header: React.FC<HeaderProps> = ({ onLogoClick }) => {
  ...
  const { profile, isTelegramWebApp, isLoading } = useTelegramAuth();
  const [showTelegramModal, setShowTelegramModal] = useState(false);
  
  const requiresTelegram = !isLoading && !isTelegramWebApp;
  ...
  return (
    <>
      ...
      <header className="w-full max-w-6xl mb-6 flex justify-between items-center border-b border-foreground/10 pb-4">
        ...
        {/* Profile Button - Always visible, shows modal if not in Telegram */}
        <button
